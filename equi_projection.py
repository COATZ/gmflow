...
# import policies.create_offset_tensor as cot
...
def show_clara_proj(center0, r, alpha, r_scale):
    ...
    print('ATTENTION ANGLE MODIFIE A LA MAIN!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
    phi0 = (center0[0]-width/2)*2*np.pi/width-np.deg2rad(45)
    ...
    for j in np.arange((r-1)/2, -(r-1)/2-1, -1):
        for i in np.arange(-(r-1)/2, (r-1)/2+1, 1):
            ...
            temp2 = [np.cos(phi0)*r_scale*i+np.sin(phi0)*(np.sin(-omega0)*r_scale*j+np.cos(-omega0)*d), np.cos(-omega0)*r_scale*j-np.sin(-omega0)*d,
    ...
    print(p)
    for elt in p:
        text_str = r"\def\xtomodif{("
        for ielt in elt:
            text_str = text_str + str(ielt) + ', '
        text_str += ")}"
        print((text_str))
    # import sys
    # sys.exit()
    ...
    ref_ij = []
    for elt in ptild:
        ref_ij.append([center0[0]+elt[0], center0[1]+elt[1]*r_scale2])
...
if __name__ == "__main__":
    ...
    # center1 = [3*width/4, height/12]
    # center1 = [230, height/2]
    center2 = [4*width/5, 2*height/8]
    # center0 = [40, 20]
    # center0 = [768, 350]
    center0 = [width/2, height/2]

    # center0 = [width/2, height/2]
    # center1 = [1.2*width/5, height/15]
    
    ## For 085_rgb.png
    center0 = [width/2+87, height/2]
    center1 = [3*width/4+33, 1*height/7]
    center2 = [width/2, 11.2*height/12]

    ## For 028_rgb.png
    # center0 = [width/2, height/2]
    # center1 = [7*width/12, 1*height/15]
    # center2 = [width/2, 10*height/12]
    # center2 = [width/2, 11.2*height/12]

    r_scale = 1
    r_scale = 30
    # r_scale = 150 ## TO GET THE PLANE
    ...
    #print(p0, u_loc)
    # print(center0[0])
    #off_x, off_y = cot.equi_coord(1000,1000,3,3,center0[0],center0[1])
    # off_x, off_y = cot.equi_coord(100,100,3,3,0,10)
    #print(off_x, off_y)
    #for elt in off_x:
    #   # plt.scatter(2*r_scale*off_x+center0[0], 2*r_scale*off_y+center0[1], color='y', s=5)
    #    plt.scatter(off_x+center0[0], off_y+center0[1], color='y', s=5)
    p1 = show_clara_proj(center1, r, alpha, r_scale)
    p2 = show_clara_proj(center2, r, alpha, r_scale)
    ...
    #plt.show()
    plt.axis('off')
    plt.savefig('./out_9.png', dpi=1200, bbox_inches='tight', pad_inches=0)
